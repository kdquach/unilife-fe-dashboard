...
import { Table, Input, Tag, Switch, Space, Empty, Typography } from 'antd';
import { SearchOutlined, PictureOutlined } from '@ant-design/icons';

const { Search } = Input;
const { Text } = Typography;

const MenuScheduleItemsTable = ({ items, loading }) => {
  const [searchText, setSearchText] = useState('');
  const [showInactive, setShowInactive] = useState(false);

  // useDeferredValue to prevent UI freezing during typing when filtering large lists
  const deferredSearchText = useDeferredValue(searchText);

  // Filter items based on deferred search text and inactive toggle
